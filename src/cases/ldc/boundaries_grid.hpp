#ifndef BOUNDARIES_GRID_HPP
#define BOUNDARIES_GRID_HPP

#include "../../var.hpp"

inline unsigned int boundary_definitions_coarse(const size_t x, const size_t y)
{
    if (x == 0 && y == 0)
    {
#ifdef WITH_CORNERS
...
inline unsigned int boundary_definitions_fine(const size_t x, const size_t y)
{
    if (x == 0 && y == 0)
    {
        return NODE_4;
...
inline void boundary_condition_coarse(unsigned int nodeType, dfloat (&pop)[Q], dfloat *rhoVar, dfloat *ux, dfloat *uy, dfloat *mxx, dfloat *myy, dfloat *mxy)
{
    switch (nodeType)
    {
    case NORTH:
...
inline void boundary_condition_fine(unsigned int nodeType, unsigned int *iscopied, dfloat (&pop)[Q], dfloat *rhoVar, dfloat *ux, dfloat *uy, dfloat *mxx, dfloat *myy, dfloat *mxy)
{
    switch (nodeType)
    {
    case NORTH:
